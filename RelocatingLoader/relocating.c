...
{
    char line[100];
    int actual_start, program_start = 0, relocation = 0;
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        else if (line[0] == 'T')
        {
            ...
            int text_start = (int)strtol(start, NULL, 16);
            int text_addr = actual_start + (text_start - program_start);
            ...
            while (i < len && obj[i] != '$')
            {
                if (obj[i] != '^')
                {
                    ...
                    // ✅ apply relocation only if bitmask bit = 1
